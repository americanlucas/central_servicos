...
@bp_serv.route('/salvar_incluir', methods=['POST'])  # /adm/serv/salvar_incluir
def salvar_incluir():
    dao = ServicoDAO()
    serv = dao.new_object()
...
def atualizar():
    dao_setor = SetorDAO()
    setores = dao_setor.read_by_filters([('sts_setor', '=', 'A')])
    return render_template('adm/serv/atualizar.html', servicos=[], setores=setores, filtro_usado='')
...
def roda_atualizar():
    ...
    dao_setor = SetorDAO()
    setores = dao_setor.read_by_filters([('sts_setor', '=', 'A')])
    return render_template('adm/serv/atualizar.html', servicos=servicos, setores=setores, filtro_usado=filtro_usado)


@bp_serv.route('/excluir/<int:idt>')
def excluir(idt):
    dao = ServicoDAO()
    if dao.delete(idt):
...
def alterar(idt):
   ...
   dao_setor = SetorDAO()
   setores = dao_setor.read_by_filters([('sts_setor', '=', 'A')])
   return render_template('adm/serv/alterar.html', msg="", css_msg="", servico=servico, setores=setores)
...
def salva_alterar():
   ...
   return render_template('adm/serv/alterar.html', msg=msg, css_msg=css_msg, servico=servico, setores=setores)