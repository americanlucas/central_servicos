...
def consultar():
   ...
   for obj in objs:
       print(obj.idt_servico, obj.nme_servico, obj.num_dias_servico, obj.vlr_servico, obj.txt_modelo_servico, obj.sts_servico, obj.cod_setor)
...
def sql():
   dao = ServicoDAO()
   objs = dao.execute_sql_and_fetch('select nme_setor, count(idt_servico) as conta from tt_setor left join tb_servico on idt_setor=cod_setor group by nme_setor')
   print("Dados da consulta:")
   for obj in objs:
       print(obj)
